...
def test_classifier_sees_the_pending_state_and_a_bounded_message():
    long_contract = "Điều 1. " + ("nội dung " * 2000)
    client = FakeAIClient(_llm({"intent": "REVIEW"}))
    ...
    # would bill the tenant for tokens that cannot change the label.
    assert payload["message"] == long_contract[:CLASSIFIER_MESSAGE_LIMIT]
